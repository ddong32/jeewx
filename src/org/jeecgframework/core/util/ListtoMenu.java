...
public class ListtoMenu {
    ...
    static void iterGet(List<TSFunction> set1, String pid, StringBuffer buffer) {
        for (TSFunction node : set1) {
            ...
            if (node.getTSFunction().getId().equals(pid)) {
                buffer.append("{\"menuid\":\"" + node.getId() + " \",\"icon\":\"" + ResourceUtil.allTSIcons.get(node.getTSIcon().getId()).getIconClas() + "\"," + "\"menuname\":\""
                        + getMutiLang(node.getFunctionName()) + "\",\"url\":\"" + node.getFunctionUrl() + "\"");
                if (count == set1.size()) {
                    buffer.append("}\n");
                }
                buffer.append("},\n");
    ...
    public static String getBootMenu(List<TSFunction> pFunctions, List<TSFunction> functions) {
        ...
        for (TSFunction pFunction : pFunctions) {
            ...
            for (TSFunction function : functions) {
                if (function.getTSFunction().getId().equals(pFunction.getId())) {
                    menuString.append("<li><a href=\"" + function.getFunctionUrl() + "\" target=\"contentiframe\"><span class=\"icon16 icomoon-icon-file\"></span>"
        ...
        for (TSFunction pFunction : pFunctions) {
            ...
            for (TSFunction function : functions) {
                if (function.getTSFunction().getId().equals(pFunction.getId())) {
                    ...
                    menuString.append("<li><div onclick=\"addTab(\'" + getMutiLang(function.getFunctionName()) + "\',\'" + funUrl + "\',\'" + icon + "\')\"  title=\""
                            + getMutiLang(function.getFunctionName()) + "\" url=\"" + function.getFunctionUrl() + "\" iconCls=\"" + icon + "\"> <a class=\"" + getMutiLang(function.getFunctionName())
                            + "\" href=\"#\" > <span class=\"icon " + icon + "\" >&nbsp;</span> <span class=\"nav\" >" + getMutiLang(function.getFunctionName()) + "</span></a></div></li>");
                }
            }
            if (hasSubfun) {
                menuString.append("</ul></div>");
        ...
        for (TSFunction function : list) {
            if (function.getTSFunction().getId().equals(parent.getId())) {
                if (!function.hasSubFunction(map)) {
                    ...
                    menuString
                            .append("<div title=\"" + getMutiLang(function.getFunctionName()) + "\" iconCls=\"" + ResourceUtil.allTSIcons.get(function.getTSIcon().getId()).getIconClas() + "\"><ul>");
    ...
    public static String getBootstrapMenu(Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction pFunction : pFunctions) {
            ...
            menuString.append("			<span class=\"bootstrap-icon\" style=\"background-image: url('" + ResourceUtil.allTSIcons.get(pFunction.getTSIcon().getId()).getIconPath() + "')\"></span> "
    ...
    private static String getBootStrapChild(TSFunction parent, int level, Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction function : list) {
            if (function.getTSFunction().getId().equals(parent.getId())) {
                ...
                menuString.append("		<li onclick=\"showContent(\'" + getMutiLang(function.getFunctionName()) + "\',\'" + menu_url + "\')\"  title=\"" + getMutiLang(function.getFunctionName())
    ...
    public static String getWebosMenu(Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction pFunction : pFunctions) {
            ...
            menuString.append("{\"id\":\"" + pFunction.getId() + "\",\"name\":\"" + pFunction.getFunctionName() + "\",\"path\":\""
                    + ResourceUtil.allTSIcons.get(pFunction.getTSIcon().getId()).getIconPath() + "\",\"level\":\"" + pFunction.getFunctionLevel() + "\",");
    ...
    private static String getWebosChild(TSFunction parent, int level, Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction function : list) {
            if (function.getTSFunction().getId().equals(parent.getId())) {
                boolean hasSub = function.hasSubFunction(map);
                menuString.append("\"" + function.getId() + "\":");
                menuString.append("{\"id\":\"" + function.getId() + "\",\"name\":\"" + getMutiLang(function.getFunctionName()) + "\",\"path\":\"" + function.getTSIcon().getIconPath() + "\",\"url\":\""
    ...
    public static String getDIYMultistageTree(Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction function : list) {
             ...
             * menuString.
             * append("<b class=\"arrow\"></b><ul  class=\"submenu\" >"); }
