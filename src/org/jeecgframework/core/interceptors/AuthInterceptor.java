...
public class AuthInterceptor implements HandlerInterceptor {
            ...
            if (client != null && client.getUser() != null) {
                ...
                if (!oConvertUtils.isEmpty(functionId)) {
                    ...
                    if (allOperation.size() > 0) {
                        ...
                        String hasOperSql = "SELECT operation FROM t_s_role_function fun, t_s_role_user role WHERE " + "fun.functionid='" + functionId + "' AND fun.operation!='' AND fun.roleid=role.roleid AND role.userid='" + client.getUser().getId() + "' ";
    ...
    private boolean hasMenuAuth(HttpServletRequest request) {
        ...
        String userid = ClientManager.getInstance().getClient(ContextHolderUtils.getSession().getId()).getUser().getId();
        // requestPath=requestPath.substring(0, requestPath.indexOf("?")+1);
        String sql = "SELECT DISTINCT f.id FROM t_s_function f,t_s_role_function  rf,t_s_role_user ru " + " WHERE f.id=rf.functionid AND rf.roleid=ru.roleid AND " + "ru.userid='" + userid
