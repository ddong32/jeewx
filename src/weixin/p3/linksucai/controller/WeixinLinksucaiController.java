...
import weixin.p3.oauth2.rule.RemoteWeixinMethodI;
...
public class WeixinLinksucaiController extends BaseController {
    ...
    @Autowired
    private RemoteWeixinMethodI remoteWeixinMethod;
    @Autowired
    ...
    public void privateDatagrid(WeixinLinksucaiEntity weixinLinksucai, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        // update-begin--Author:macaholin Date:20150404
        // for：新增微信公众账号邮编名称（不存储数据库，只有用于显示），用于区分相同素材标题情况下是哪个父亲账号分享下来的
        List<WeixinAccountEntity> accountList = weixinAccountService.loadAll(WeixinAccountEntity.class);
        // update-end--Author:macaholin Date:20150404
        // for：新增微信公众账号邮编名称（不存储数据库，只有用于显示），用于区分相同素材标题情况下是哪个父亲账号分享下来的
    ...
    public void link(WeixinLinksucaiEntity weixinLinksucai, HttpServletRequest request, HttpServletResponse response) {
        ...
        // update-start--Author:scott Date:20150809
        // for：【判断访问地址是否有附加参数，有的话原样带回去】----------------------
    ...
    public AjaxJson doBatchDel(String ids, HttpServletRequest request) {
        ...
        try {
            for (String id : ids.split(",")) {
                WeixinLinksucaiEntity weixinLinksucai = systemService.getEntity(WeixinLinksucaiEntity.class, id);
                if (!weixinLinksucai.getAccountid().equals(ResourceUtil.getShangJiaAccountId())) {
                    continue;
                }
                weixinLinksucaiService.delete(weixinLinksucai);
    ...
    public void exportXls(WeixinLinksucaiEntity weixinLinksucai, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        try {
            ...
            workbook = ExcelExportUtil.exportExcel(new ExcelTitle("链接素材列表", "导出人:" + ResourceUtil.getSessionUserName().getRealName(), "导出信息"), WeixinLinksucaiEntity.class, weixinLinksucais);
    ...
    public void exportXlsByT(WeixinLinksucaiEntity weixinLinksucai, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        try {
            ...
            // 产生工作簿对象
            HSSFWorkbook workbook = null;
            workbook = ExcelExportUtil.exportExcel(new ExcelTitle("链接素材列表", "导出人:" + ResourceUtil.getSessionUserName().getRealName(), "导出信息"), WeixinLinksucaiEntity.class, null);
            fOut = response.getOutputStream();
            workbook.write(fOut);
        } catch (Exception e) {
        } finally {
    ...
    public AjaxJson importExcel(HttpServletRequest request, HttpServletResponse response) {
        ...
        for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
            ...
            try {
                List<WeixinLinksucaiEntity> listWeixinLinksucaiEntitys = (List<WeixinLinksucaiEntity>) ExcelImportUtil.importExcelByIs(file.getInputStream(), WeixinLinksucaiEntity.class, params);
    ...
    public ModelAndView poplink(ModelMap modelMap, @RequestParam String id) {
        // WeixinLinksucaiEntity weixinLinksucai =
        // weixinLinksucaiService.getEntity(WeixinLinksucaiEntity.class, id);
