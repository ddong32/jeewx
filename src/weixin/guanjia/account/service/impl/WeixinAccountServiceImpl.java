...
public class WeixinAccountServiceImpl extends CommonServiceImpl implements WeixinAccountServiceI {
    ...
    public String getAccessToken() {
        ...
        if (token != null && !"".equals(token)) {
            ...
            java.util.Date start = new java.util.Date(account.getAddtoekntime().getTime());
    ...
    public String getAccessToken(String accountId) {
        ...
        if (token != null && !"".equals(token)) {
            ...
            if ((end.getTime() - start.getTime()) / 1000 / 3600 >= 2) {
                // 失效 重新获取
                String requestUrl = WeixinUtil.access_token_url.replace("APPID", weixinAccountEntity.getAccountappid()).replace("APPSECRET", weixinAccountEntity.getAccountappsecret());
                JSONObject jsonObject = WeixinUtil.httpRequest(requestUrl, "GET", null);
                if (null != jsonObject) {
                    try {
                        token = jsonObject.getString("access_token");
    ...
    public WeixinAccountEntity findByToUsername(String toUserName) {
        return this.findUniqueByProperty(WeixinAccountEntity.class, "weixin_accountid", toUserName);
    ...
    public AjaxJson resetAccessToken(String accountid) throws WexinReqException {
        ...
        if (null != jsonObject) {
            ...
            try {
                ...
                try {
                    // [2].获取api凭证
                    // GetticketRtn getticketRtn =
                    // JwQrcodeAPI.doGetticket(token);
                    ...
                    // } catch (Exception e) {
                    // // 获取api凭证失败
                    // String wrongMessage = "获取api凭证失败 errcode:{"+
                    // getticketRtn.getErrcode()+"}
                    // errmsg:{"+getticketRtn.getErrmsg()+"}";
                ...
                try {
                    ...
                    if (null != jsapi_ticket_json) {
                        try {
                            ...
                            // 获取jsapi凭证失败
                            String wrongMessage = "获取jsapi凭证失败 errcode:{" + (jsonObject.containsKey("errcode") ? jsonObject.get("errcode") : "") + "} errmsg:{"
                                    + (jsonObject.containsKey("errmsg") ? jsonObject.getString("errmsg") : "") + "}";
                            LogUtil.info(wrongMessage);
                        }
                    }
                } catch (Exception e) {
