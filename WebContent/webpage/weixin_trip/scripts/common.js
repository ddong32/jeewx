...
function visaBox(){
   if($(".boxvisa").length>0){
	  $(".boxvisa .va_item .va_tom").click(function(){
		 $(this).toggleClass("cur");
		 $(this).next(".va_tum").slideToggle(); 
	  })    
   }	
   if($(".feebox").length>0){
      $(".feebox .feebox_item .feebox_tit").click(function(){
		 $(this).toggleClass("cur");
		 $(this).next(".feebox_txt").slideToggle(); 
	  })
   }
}
function headTop(){
...
function mePage(id,fun){
    var BackBtn=null,name=$(".header h1.es").text(),foot=$("#footer").is(":hidden");
    //事件集合
    var Fn = {
        backFn: function (e) {
            if (BackBtn != null) {
...
//验证姓名
function checkPerName(obj,type){
    var tj = true;
    if($("#" + obj).val()!=""){
       if(/^(.*?)+[\d~!@@#$%^&*()_\-+\={}\[\];:'"\|,.<>?！￥……（）——｛｝【】；：'"'"、《》，。、？]/.test($("#" + obj).val())){
		  if(type){
...
//验证手机号码
function checkPerPhone(obj,type) {
    var tj=true;
    if($("#" + obj).val()!=""){
